'''
homework_1.py
This is a program that asks the user for their dream job title.  Then asks
for the hourly wage they want to earn.  This number accounts
...
over 52 weeks. (Assuning vacation time is unpaid.) Print weekly and annual
salary. Then asks how much money
they feel they will need at retirement time.  Determine the number of
years they will have to work to save up their retirement dollar value
assuming their only source of income was their paycheck.  Finally,
determine if the number of years they need to save is an even or odd number.
For this part, use the modulus operator to determine if the number you
calculated is odd or even.  Once you have calculated it, use print to
print out the value.  You should also print out something like this.
"If the value is --- then the number is odd, however, if the value
is ----- then the number is even."
...
try:
    ...
    print 'The value provided' ,input_hourly_wage, 'is NOT a valid number. Exiting program.'
else:

# test wage for negative numbers and 0

    if float(input_hourly_wage) <=0:
        print input_hourly_wage, 'is NOT a valid value. Exiting program.'
    else:
...
# test retirement money for string input
        ...
        try:
            ...
            print ' '
            print 'The input provided' ,input_retirement_money, 'is NOT a valid number. Exiting program.'
        else:
            
# test retirement money for negative numbers and 0

            if float(input_retirement_money) <=0:
                print input_retirement_money, 'is NOT a valid number. Exiting program.'
            else:
